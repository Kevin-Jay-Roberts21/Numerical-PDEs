...
exact0 = box_function(x, a, b);
u0 = box_function(x, a, b);
d0 = x./(x.^4 + 1);
c0 = x./(x.^4 + 1);
leap0 = box_function(x, a, b);
crank0 = box_function(x, a, b);
lax0 = box_function(x,a,b);
up30 = box_function(x,a,b);
...
function u_nonsmooth = box_function(x, a, b)
    u_nonsmooth = zeros(size(x));
    u_nonsmooth(x >= a & x <= b) = 1;  % Box function between a and b
end
...
for n = 1:N % beginning the for time loop
    ...
    % plotting one line
    if n == round(T/dt)/4 % check if current time is close to a multiple of 2
